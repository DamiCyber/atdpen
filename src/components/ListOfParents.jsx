...
import "../assets/style/listofparent.css";
...
const ListOfParents = () => {
    ...
    if (!parents || parents.length === 0) return <div className="error-container">No parents found</div>;
            ...
            <div className={`main-content ${isSidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
                <div className="header">
                    <div className="header-left">
                        <h1 className="dashboard-title">List of Parents</h1>
                    </div>
                            ...
                            {parents.map((parent) => (
                                <div key={parent.id} className="parent-card">
                                        ...
                                        {profilePictures[parent.id] ? (
                                            <img
                                                ...
                                                onError={(e) => {
                                                    e.target.onerror = null;
                                                    e.target.src = null;
                                                    e.target.parentElement.innerHTML = `<div class="profile-placeholder">${parent.name?.charAt(0)?.toUpperCase() || 'P'}</div>`;
                                                }}
                                            />
                                        ) : (
                                            <div className="profile-placeholder">
                                    ...
                                    <div className="parent-info">
                                        <h3 className="parent-name">{parent.name || 'Unknown Parent'}</h3>
                                        <div className="parent-details">
                                            <p><span>Email:</span> {parent.email || 'N/A'}</p>
                                            <p><span>Phone:</span> {parent.phone_number || 'N/A'}</p>
                                            <p><span>Address:</span> {parent.address || 'N/A'}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
